...
module.exports = function(options) {
    ...
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function() {
          ...
          .then(function() {
            dbapi.validateUser(req.body)
              .then(function (isValidate) {
                if (isValidate) {
                  log.info('IsLoggedIn "%s" "%s"', req.body.email,req.body.name)
                  ...
                  log.info('IsNotLoggedIn "%s" "%s"', req.body.email,req.body.name)
                  res.status(200)
                    .json({
                      success: false
                      , redirect: '/auth/mock/'
  ...
  app.post('/auth/api/v1/mock2', function (req, res) {
    var log = logger.createLogger('auth-mock')
    log.setLocalIdentifier(req.ip)
    switch (req.accepts(['json'])) {
      case 'json':
