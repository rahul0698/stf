...
module.exports.handler = function(argv) {
  ...
  function run() {
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--storage-url'
        , util.format('http://192.168.104.71:%d/', argv.port)
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--port', argv.port
        , '--app-url'
        , util.format('http://192.168.104.71:%d/', argv.appPort)
        , '--auth-url'
        , util.format('http://192.168.104.71:%d/', argv.authPort)
        , '--api-url'
        , util.format('http://192.168.104.71:%d/', argv.apiPort)
        , '--websocket-url'
        , util.format('http://192.168.104.71:%d/', argv.websocketPort)
        , '--storage-url'
        , util.format('http://192.168.104.71:%d/', argv.storagePort)
        , '--storage-plugin-image-url'
        , util.format('http://192.168.104.71:%d/', argv.storagePluginImagePort)
        , '--storage-plugin-apk-url'
        , util.format('http://192.168.104.71:%d/', argv.storagePluginApkPort)
        ])
    ]
